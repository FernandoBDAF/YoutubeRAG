...
def export_graph_csv(
    ...
    writer.writerow(["id", "name", "canonical_name", "type", "description", "confidence", "source_count"])
...
def export_graph_graphml(
    ...
    for i, link in enumerate(graph_data["links"]):
        ...
        xml_parts.append(f'      <data key="predicate">{_escape_xml(link.get("predicate", ""))}</data>')
...
def export_graph_gexf(
    ...
    # Add nodes
    for node in graph_data["nodes"]:
        xml_parts.append(f'      <node id="{node["id"]}" label="{_escape_xml(node.get("name", ""))}">')
        xml_parts.append('        <attvalues>')
        xml_parts.append(f'          <attvalue for="type" value="{_escape_xml(node.get("type", ""))}"/>')
        xml_parts.append(f'          <attvalue for="name" value="{_escape_xml(node.get("name", ""))}"/>')
        xml_parts.append('        </attvalues>')
        xml_parts.append('      </node>')
    ...
    for i, link in enumerate(graph_data["links"]):
        xml_parts.append(f'      <edge id="e{i}" source="{link["source"]}" target="{link["target"]}">')
        xml_parts.append('        <attvalues>')
        xml_parts.append(f'          <attvalue for="predicate" value="{_escape_xml(link.get("predicate", ""))}"/>')
        xml_parts.append(f'          <attvalue for="confidence" value="{link.get("confidence", 0)}"/>')
        xml_parts.append('        </attvalues>')
        xml_parts.append('      </edge>')
        ...
        try:
            if len(path_parts) >= 3 and path_parts[0] == "api" and path_parts[1] == "export":
                ...
                if format_type == "json":
                    ...
                    self.send_header("Content-Disposition", f'attachment; filename="graph_{db_name}.json"')
                    self.end_headers()
                    ...
                    self.send_header("Content-Disposition", f'attachment; filename="graph_{db_name}.csv"')
                    self.end_headers()
                    ...
                    self.send_header("Content-Disposition", f'attachment; filename="graph_{db_name}.graphml"')
                    self.end_headers()
                    self.wfile.write(result.encode("utf-8"))
                    ...
                    self.send_response(200)
                    self.send_header("Content-Type", "application/xml")
                    self.send_header("Content-Disposition", f'attachment; filename="graph_{db_name}.gexf"')
                    self.end_headers()
                    self.wfile.write(result.encode("utf-8"))
                else:
