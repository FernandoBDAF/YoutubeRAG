...
def fetch_transcript_text(video_id: str) -> Tuple[Optional[str], Optional[str]]:
    try:
        # Try English first; fall back to any available
        transcript_list = YouTubeTranscriptApi.list_transcripts(video_id)
        text = None
        lang = None
        try:
            tr_en = transcript_list.find_transcript(["en", "en-US", "en-GB"])  # type: ignore
            entries = tr_en.fetch()
            lang = tr_en.language
        except Exception:
            tr = transcript_list.find_manually_created_transcript(transcript_list._generated_transcripts.keys() or ["en"])  # type: ignore
            entries = tr.fetch()
            lang = tr.language
